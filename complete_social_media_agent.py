...
class CompleteSocialMediaAgent:
    ...
    def create_fixtures_post(self, team_data: Dict[str, Any], fixtures: List[Dict[str, Any]], 
        ...
        import os
        try:
            # First try: Use bundled fonts from assets/fonts (works everywhere)
            font_dir = os.path.join('assets', 'fonts')
            team_font = ImageFont.truetype(os.path.join(font_dir, "LiberationSans-Bold.ttf"), 32)
            opponent_font = ImageFont.truetype(os.path.join(font_dir, "LiberationSans-Regular.ttf"), 24)
            venue_font = ImageFont.truetype(os.path.join(font_dir, "LiberationSans-Regular.ttf"), 14)
            vs_font = ImageFont.truetype(os.path.join(font_dir, "LiberationSans-Bold.ttf"), 22)
        except:
            try:
                # Second try: Arial Bold (Windows)
                team_font = ImageFont.truetype("arialbd.ttf", 32)
                ...
                try:
                    # Third try: Segoe UI (Windows)
                    team_font = ImageFont.truetype("seguibl.ttf", 32)
                    opponent_font = ImageFont.truetype("segoeui.ttf", 24)
                    venue_font = ImageFont.truetype("segoeui.ttf", 14)
                    vs_font = ImageFont.truetype("seguisb.ttf", 22)
                except:
                    try:
                        # Fourth try: DejaVu or Liberation (Linux system fonts)
                        team_font = ImageFont.truetype("/usr/share/fonts/liberation/LiberationSans-Bold.ttf", 32)
                        opponent_font = ImageFont.truetype("/usr/share/fonts/liberation/LiberationSans-Regular.ttf", 24)
                        venue_font = ImageFont.truetype("/usr/share/fonts/liberation/LiberationSans-Regular.ttf", 14)
                        vs_font = ImageFont.truetype("/usr/share/fonts/liberation/LiberationSans-Bold.ttf", 22)
                    except:
                        # Final fallback - use default fonts but with correct sizes
                        print("   ⚠️  Using default fonts - install Liberation fonts for better quality")
                        team_font = self.title_font  # Use larger default
    ...
    def create_results_post(self, team_data: Dict[str, Any], results: List[Dict[str, Any]], 
        ...
        try:
            # First try: Use bundled fonts from assets/fonts (works everywhere)
            font_dir = os.path.join('assets', 'fonts')
            date_font = ImageFont.truetype(os.path.join(font_dir, "LiberationSans-Bold.ttf"), 18)
            result_font = ImageFont.truetype(os.path.join(font_dir, "LiberationSans-Bold.ttf"), 28)
            form_font = ImageFont.truetype(os.path.join(font_dir, "LiberationSans-Bold.ttf"), 28)
        except:
            try:
                # Second try: Arial Bold (Windows)
                date_font = ImageFont.truetype("arialbd.ttf", 18)
                result_font = ImageFont.truetype("arialbd.ttf", 28)
                form_font = ImageFont.truetype("arialbd.ttf", 28)
            except:
                try:
                    # Third try: Segoe UI (Windows)
                    date_font = ImageFont.truetype("seguisb.ttf", 18)
                    result_font = ImageFont.truetype("seguibl.ttf", 28)
                    form_font = ImageFont.truetype("seguibl.ttf", 28)
                except:
                    try:
                        # Fourth try: Liberation (Linux system fonts)
                        date_font = ImageFont.truetype("/usr/share/fonts/liberation/LiberationSans-Bold.ttf", 18)
                        result_font = ImageFont.truetype("/usr/share/fonts/liberation/LiberationSans-Bold.ttf", 28)
                        form_font = ImageFont.truetype("/usr/share/fonts/liberation/LiberationSans-Bold.ttf", 28)
                    except:
    ...
    def create_weekly_results_post(self, all_results: List[Dict[str, Any]], 
        ...
        if all_results:
            for i, result in enumerate(all_results[:20]):  # Max 20 results
                ...
                # Determine age group for score display
                team_name_for_age = home if ('scawthorpe' in home.lower() or 'scorpions' in home.lower()) else away
                age_group = get_age_group(team_name_for_age)
                ...
                # Score - show "X-X" in red for U10 and below
                if age_group <= 10:
                    score_text = "X-X"
                ...
                draw.text((x_current, y_result), score_text, fill=score_color, font=result_font)
